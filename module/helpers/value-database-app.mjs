...
export class ValueDatabaseApp extends ApplicationV2 {
  ...
  _valueControl(def) {
    ...
    if (def.type === "boolean") return `<input type="checkbox" data-value-id="${esc(def.id)}" ${value ? "checked" : ""}>`;
    if (def.type === "color") return `<input type="color" data-value-id="${esc(def.id)}" value="${esc(value || "#7aa2ff")}">`;
    if (["array", "object"].includes(def.type)) {
      const text = typeof value === "string" ? value : JSON.stringify(value ?? (def.type === "array" ? [] : {}));
      return `<textarea data-value-id="${esc(def.id)}" rows="2">${esc(text)}</textarea>`;
    }
    const numeric = ["number", "integer"].includes(def.type);
    return `<input type="${numeric ? "number" : "text"}" ${numeric ? 'step="any"' : ""} data-value-id="${esc(def.id)}" value="${esc(value)}"
  ...
  _draftRow(draft, index) {
    return `<div class="sd-db-draft" data-draft-index="${index}">
      ...
      <input data-draft="initial" value="${esc(draft.initial)}" placeholder="Initial value" autocomplete="off">
  ...
  async _renderHTML() {
    ...
    const rows = defs.map(def => `<div class="sd-db-row" data-variable-id="${esc(def.id)}">
      <span class="sd-db-row-name"><i class="fas fa-cube"></i><b>${esc(def.name)}</b><small>${esc(def.id)} · ${esc(def.type)} · ${esc(def.scope)}</small></span>
    ...
    return `<div class="sd-db-app">
      ...
      <footer class="sd-db-foot">
        <button type="button" class="sd-db-secondary" data-action="addVariable"><i class="fas fa-plus"></i> Add Variable</button>
