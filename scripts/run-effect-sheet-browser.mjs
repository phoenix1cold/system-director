...
page.on('console',m=>{if(m.text().startsWith('dbg'))console.log(m.text());if(m.type()==='error'&&!m.text().includes('404'))console.error('[page]',m.text());});page.on('pageerror',e=>console.error('[pageerror]',e.message));
await page.goto(`http://127.0.0.1:${server.address().port}/scripts/test-effect-sheet-browser.html`);
await page.waitForFunction(()=>['PASS','FAIL'].includes(document.title),null,{timeout:60000});await page.waitForTimeout(300);
await page.locator('.application').screenshot({path:root+'/tests/effect-sheet-shell.png'});
...
if(title!=='PASS'){process.exitCode=1;console.error('effect sheet browser: FAIL');}else console.log('effect sheet browser: PASS');
