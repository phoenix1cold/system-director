...
const catalog=JSON.parse(fs.readFileSync(new URL("../../sd-wiki-server/seed/node-catalog.json",import.meta.url),"utf8"));
const nodes=catalog.nodes&&typeof catalog.nodes==="object"?catalog.nodes:catalog;
...
for(const id of ['on_click','act_roll_v2','act_present_roll']){if(!nodes[id])throw Error(`tutorial node missing from catalog: ${id}`);if(!guide.includes(`data-type='${id}'`))throw Error(`tutorial does not reference ${id}`);}
