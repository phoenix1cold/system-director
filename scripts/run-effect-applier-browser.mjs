...
await page.goto(`http://127.0.0.1:${server.address().port}/scripts/test-effect-applier-browser.html`);
await page.waitForFunction(()=>['PASS','FAIL'].includes(document.title),null,{timeout:60000});await page.evaluate(()=>{const f=document.querySelector('[data-action="filter"]');f.value='';f.dispatchEvent(new Event('input'));document.querySelectorAll('[data-preset]')[0].click();});await page.waitForTimeout(300);
await page.locator('.application').screenshot({path:root+'/tests/effect-applier-shell.png'});
...
if(title!=='PASS'){process.exitCode=1;console.error('effect applier browser: FAIL');}else console.log('effect applier browser: PASS');
