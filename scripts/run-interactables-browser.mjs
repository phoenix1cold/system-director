...
await page.goto(`http://127.0.0.1:${server.address().port}/scripts/test-interactables-browser.html`);
await page.waitForFunction(()=>['PASS','FAIL'].includes(document.title),null,{timeout:60000});await page.waitForTimeout(300);
await page.locator('.application').screenshot({path:root+'/tests/interactables-shell.png'});
...
if(title!=='PASS'){process.exitCode=1;console.error('interactables browser: FAIL');}else console.log('interactables browser: PASS');
