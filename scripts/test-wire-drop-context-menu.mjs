...
assert.match(source,/this\._ctxMenu\(clientX,clientY,gx,gy,conn\)/,"dropping a wire on empty canvas must open the RMB context menu");
...
assert.match(source,/doc\.querySelector\("\.sdgctx"\)\?\.remove\(\)/,"wire and RMB paths must share the detached-window-aware sdgctx menu implementation");
