...
Пакеты версии 0.19 могут включать `localizationLanguages`, вложенные карты переводов `i18n`, `effectPresets` и описание возможностей пакета. При импорте языки и пресеты эффектов объединяются с локальными библиотеками, не удаляя несвязанные записи.
